...
const renderReservationCard = (reservation, index) => {
  ...
  const disabled = status === 'past' || status === 'waiting' ? 'disabled' : '';
  const statusText = status === 'waiting'
    ? `대기 ${reservation.sequence}번`
    : status === 'past' ? '지난 예약' : '변경 가능';
  const cancelText = status === 'waiting' ? '대기 취소' : '예약 취소';

  return `
    <article class="my-reservation-card" data-id="${reservation.id}" data-theme-id="${reservation.theme.id}" data-status="${reservation.status || 'RESERVED'}">
      ...
      <div class="my-reservation-body">
        ...
        <div class="my-actions">
          ...
          <button class="btn-link cancel-reservation" type="button" ${status === 'past' ? 'disabled' : ''}>${cancelText}</button>
        </div>
      </div>
    </article>
  `;
...
const hydrateCards = () => {
  ...
  if (!reservation || getReservationStatus(reservation) !== 'active') return;
...
const cancelReservation = async (card) => {
  ...
  if (!confirm(`${reservation.theme.name} ${isWaiting ? '대기' : '예약'}를 취소할까요?`)) return;
