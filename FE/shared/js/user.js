  ...
  try {
    ...
    timeSlotsContainer.innerHTML = times.map(time => `
      <div class="time-slot ${time.available ? '' : 'waiting'}" 
           ...
           data-waiting="${time.alreadyBooked}">
        <strong>${time.startAt}</strong>
        <span>${time.available ? '예약 가능' : '대기 가능'}</span>
      </div>
    `).join('');
...
timeSlotsContainer.addEventListener('click', (e) => {
  ...
  selectedTime = {
    ...
    waiting: slot.dataset.waiting === 'true'
  };
  
  nextBtn.disabled = false;
  reserveBtn.textContent = selectedTime.waiting ? '대기 신청하기' : '예약하기';
});
...
reserveBtn.addEventListener('click', async () => {
  ...
  reserveBtn.textContent = selectedTime.waiting ? '대기 처리 중...' : '예약 처리 중...';
  ...
  try {
    ...
    const result = selectedTime.waiting
    ...
    document.getElementById('reservation-summary').innerHTML = `
      ...
      <p><strong>상태:</strong> ${selectedTime.waiting ? `대기 ${result.sequence}번` : '예약 완료'}</p>
    `;
