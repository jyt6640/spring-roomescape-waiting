...
import roomescape.reservation.infrastructure.ReservationJdbcTemplateRepository;
...
class WaitingJdbcTemplateRepositoryTest {
    ...
    void setUp() {
        ...
        savedReservation = reservationRepository.save(Reservation.create("브라운", DATE, savedTime, savedTheme));
    }
    ...
    void save_success() {
        // given
        Waiting waiting = Waiting.create(savedReservation.getId(), "브라운", DATE, savedTime, savedTheme, 1);
        ...
        assertThat(savedWaiting.getReservationId()).isEqualTo(savedReservation.getId());
    ...
    void countByReservationId_success() {
        ...
        // when
        int count = waitingRepository.countByReservationId(savedReservation.getId());
    ...
    void findByName_success_with_current_sequence() {
        // given
        Waiting first = waitingRepository.save(Waiting.create(savedReservation.getId(), "브라운", DATE, savedTime, savedTheme, 1));
        Waiting second = waitingRepository.save(Waiting.create(savedReservation.getId(), "리오", DATE, savedTime, savedTheme, 2));
        Waiting third = waitingRepository.save(Waiting.create(savedReservation.getId(), "포비", DATE, savedTime, savedTheme, 3));
    ...
    @Test
    @DisplayName("reservationId, name이 모두 일치하는 대기를 조회한다")
    void findByReservationIdAndName_success() {
        // given
        ...
        // when
        Optional<Waiting> result = waitingRepository.findByReservationIdAndName(savedReservation.getId(), "브라운");
